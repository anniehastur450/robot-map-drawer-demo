...
const fallbackConfig = {
  ...
  mapUnit: 'm',
  bgColor: '#222',
  scaleBarPx: 100, // scale bar size is around this value
  zoomLevels: [
    1, 2, 5, 10, 15, /*
...
// TODO: correct the name, marker cover should be called cluster
// TODO: correct the name, distant indicator should be called off-screen indicator
// to fix: issue of rounded zoom, if zoom is rouned, offset have to be corrected too, currently zoom is not rounded internally, only rounded for display

class RobotMapDrawer {
  constructor(config) {
    this.config = { ...fallbackConfig, ...config };
    /* camera logic: move in map units, then zoom by percentage */
    this.camera = {
      zoom: 100,
      offset: [0, 0], // [x, y] in map units (default meters)
    };
    this.doms = {
    ...
    this._defaultUserView = new UserView(this, {});
    this.userViews = {
      default: this._defaultUserView,
    };
  }
  getUserView(options) {
    const view = new UserView(this, options);
    if (this.userViews.default === this._defaultUserView) {
      this.userViews.default = view;
    }
    return view;
  }
  createPanning() {
  ...
  panMove(...pointers) {
    ...
    this.camera.zoom = this.panning.zoom;
  ...
  registerEl(el) {
    ...
    const transition = () => {
      ...
      // TODO: real fix for zooming scale and inverse scale timing function issue, instead of this finish animation
      // because scale and reverse scale during transition their multiplication product is not constant over time
      // see this: https://stackoverflow.com/questions/54971653/css-transition-scale-weird-delay

      // console.log('before', el.getAnimations({ subtree: true }));
      el.classList.add('panning'); // adding class can also finish previous transition
      // console.log('after', el.getAnimations({ subtree: true }));
      el.classList.remove('panning');
      // el.getAnimations({ subtree: true }).forEach((x) => {
      //   if (x.transitionProperty === 'transform') {
      //     x.finish();
      //   }
      //   if (x.transitionProperty === 'width') {
      //     // scale bar
      //     x.finish();
      //   }
      // });
  ...
  requestUpdateCamera() {
    ...
    const update = () => {
      ...
      if (this.doms.zoomInput.disabled) {
        this.doms.zoom.textContent = this.getZoomText(this.camera.zoom);
      }
      this.updateScaleBar();
  ...
  getZoomText(zoom, rounded = true) {
    if (rounded) {
      zoom = zoom > 100 ? Math.round(zoom) : parseFloat(zoom.toPrecision(3)); // round it to look better
    }
    // reduce double precision to avoid precision error display as string, double is 15 to 17 significant decimal digits precision
    zoom = parseFloat(zoom.toPrecision(14));
    return `${zoom}%`;
  }
  updateScaleBar() {
    const { scaleBar, scaleText } = this.doms;
    const r = this.config.scaleBarPx / this.zoomedRatioScreenPx;
    const { width, text } = this.userViews.default.getScaleText(r);
    scaleBar.style.setProperty(
      '--scalebar-w',
      `${width * this.zoomedRatioScreenPx}px`
    );
  ...
  startInertiaDragging(previousPanning) {
    ...
    const zoomThreshold = 2.0; // zoom is 2 times than its previous second
  ...
  registerZoomInputEl(el) {
    ...
    const observer = new MutationObserver(() => {
      if (el.disabled) {
        ...
        el.value = this.getZoomText(this.camera.zoom, false);
        input();
        el.focus();
        el.select();
      }
    ...
    events({
      ...
      keydown: (e) => {
        ...
        if (el.disabled) {
          e.preventDefault(); // avoid Enter after focusing button
          this.doms.zoom.focus();
        }
      },
      input,
    }).apply(el);
    this.eventHooks.registerHooks({
      ...
      preTouchStart: () => el.blur(), // because touch prevent default do not trigger blur
    });
  }
  attach(target) {
    h`
      <div class="group/drawer w-full h-full relative font-sans">
        ...
        ${attr((el) => this.doms.ui.push({ el, region: 8 }))} >
          <div class="flex items-center">
            <div class="w-[var(--scalebar-w)] group-[:not(.panning)]/drawer:transition-width h-2 mt-1 b-2 b-solid b-black/90 b-t-none"
...
function handleAttachor(root, el, trackingActives, options) {
  const fallbackOptions = {
    finishPrevious: true, // finish previous transition
    fade: false,
  };
  const { finishPrevious, fade } = { ...fallbackOptions, ...options };
  return {
    setOpacity(op) {
      if (op === this._op) {
        return;
      ...
      if (finishPrevious) {
        // opacity timing function issue, finish previous transition to avoid blinking
        el.getAnimations().forEach((x) => {
          if (x.transitionProperty === 'opacity') {
            x.finish();
          }
        });
      }
...
class MarkerList {
  ...
  getEl() {
    this._once = !this._once || invalidAction('already initialized');
    this.drawer.eventHooks.registerHooks({
      postRender: () => this.updateMarkerCamera(),
    });
    return h`
  ...
  getMarkerDom() {
    ...
    const el = h`
      <div class="absolute -translate-1/2 scale-[calc(1/var(--s))] left-[var(--x)] top-[var(--y)] group-[:not(.panning)]/drawer:transition-transform,opacity transition-opacity opacity-[var(--op)] bg-amber/50"></div>
    `.el;
  ...
  getCoverDom() {
    ...
    const el = h`
      <div class="absolute -translate-1/2 w-[32px] h-[32px] scale-[var(--cs)] left-[var(--x)] top-[var(--y)] group-[:not(.panning)]/drawer:transition-transform,opacity transition-opacity opacity-[var(--op)] rounded-full bg-blue/50 flex justify-center items-center text-slate-700"></div>
    `.el;
...
/* provide shifting origin, axes transformation, and unit conversion support */

const fallbackUserViewOptions = {
  /*
    origin is one of:
      * 'top-left'      'top'       'top-right'
      * 'left'         'center'         'right'
      * 'bottom-left'  'bottom'  'bottom-right'
      * or any coordinates [x, y] in map unit
   */
  origin: 'center',
  '+x': 'right', // 'left' or 'right'
  '+y': 'bottom', // 'top' or 'bottom', usually it is top for math axis and bottom for computer screen axis
  viewUnit: null, // unit of this user view, use map unit if not given
  units: {
  ...
  hiddenUnits: [], // units for conversion only, will not be displayed on scale bar
...
/* round a number to the nearest number in 1-2-5 series */
function roundToPrettyNumber(num) {
  const places = 10 ** Math.floor(Math.log10(num));
  const list = [1, 2, 5, 10].map((x) => x * places);
  const closest = list.reduce(function (prev, curr) {
    return Math.abs(curr - num) < Math.abs(prev - num) ? curr : prev;
  });
  return parseFloat(closest.toPrecision(1)); // as (10 ** x) might be precision error, like (10 ** -4)
}
...
function parseAxes(positiveX, positiveY) {
  const X = { left: -1, right: 1 }[positiveX] ?? invalidArgument(positiveX);
  const Y = { top: -1, bottom: 1 }[positiveY] ?? invalidArgument(positiveY);
  return [X, Y];
}
...
class UserView {
  constructor(drawer, options) {
    this.drawer = drawer;
    this.options = { ...fallbackUserViewOptions, ...options };
    this.unit = this.options.viewUnit ?? this.drawer.config.mapUnit;
    this._init();
  }
  _init() {
    /* units */
    const parsed = parseUnits(this.options.units, this.drawer.config.mapUnit);
    const excludes = new Set(this.options.hiddenUnits);
    const scales = Object.keys(parsed).filter((x) => !excludes.has(x));
    this._parsed = parsed; // key: unit [u], value: 1 map unit = [value] [u]
    this._scales = scales;
    /* origin */
    ...
    const [X, Y] = parseAxes(this.options['+x'], this.options['+y']);
    this._X = X * parsed[this.unit];
    this._Y = Y * parsed[this.unit];
  }
  getScaleText(width) {
    // prefer converted value that >= 1, and round it to 1-2-5 series
    const values = this._scales.map((unit) => {
      const val = roundToPrettyNumber(width * this._parsed[unit]);
      return { val, unit, width: val / this._parsed[unit] };
    });
    function dis(num) {
      return Math.abs(num - 1);
    }
    values.sort((a, b) => dis(a.val) - dis(b.val));
    const f = values.find((a) => a.val >= 1) ?? values[0];
    return {
      width: f.width,
      text: `${f.val} ${f.unit}`,
    };
  ...
  setDefault() {
    this.drawer.userViews.default = this;
  }
  addMarker(id, { name, x, y, color }) {
    const [x2, y2] = this.toInternalLocation([x, y]);
    this.drawer.markerList.addMarker(id, {
      name,
      x: x2,
      y: y2,
      color,
...
// water level 3: 1809, reviewing code...

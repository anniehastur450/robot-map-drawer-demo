...
class RobotMapDrawer {
  ...
  panMove(...pointers) {
    ...
    this.camera.zoom = this.nicelyRoundedZoomLevel(this.panning.zoom);
  ...
  startInertiaDragging(previousPanning) {
    ...
    const timeout = () => {
      ...
      this.camera.zoom = this.nicelyRoundedZoomLevel(z * 100);
  ...
  updateScaleBar() {
    ...
    const width = this.config.scaleBarPx / this.zoomedRatioScreenPx;
    const scale = this.userViews.current.getScaleText(width);
    scaleBar.style.setProperty(
      '--scale-bar',
      `${scale.width * this.zoomedRatioScreenPx}px`
    );
    scaleText.textContent = `${scale.text}`;
  }

...
class RobotMapDrawer {
  constructor(config) {
    ...
    this._defaultUserViews = new UserView(this, {});
    this.userViews = {
      current: this._defaultUserViews,
    };
  }
  createPanning() {
  ...
  // nicelyRoundedZoomLevel(zoom) {
  //   // releave precision to compare for errors
  //   if (Math.fround(this.camera.zoom) === Math.fround(zoom)) {
  //     // if zoom equals to current, this might be user intended to, leaves unchanged
  ...
  updateScaleBar() {
    ...
    const { width, text } = this.userViews.current.getScaleText(r);
...
const fallbackUserViewOptions = {
  ...
  '+x': 'right', // 'left' or 'right'
  '+y': 'bottom', // 'top' or 'bottom', usually it is top for math axis and bottom for computer screen axis
  viewUnit: null, // the unit of coordinates of this user view, used in adding markers and popup information, use map unit if not given
  units: {
  ...
  hiddenUnits: [], // used by scale bar, units in this list will not be used for scale bar unit, usually this is only needed when using a different unit system, like SI units and Imperial units

...
function handleAttachor(root, el, trackingActives, options) {
  ...
  return {
    setOpacity(op, finishPrevious = true) {
      if (op === this._op) {
        return;
      }
      if (finishPrevious) {
        // TODO: real fix for opacity timing function issue, instead of this finish animation
      ...
      el.style.setProperty(opacityProperty, `${op}`);
      this._op = op;
    },
    attach() {
      /* attach with fade in */
